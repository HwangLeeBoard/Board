...
public class BoardDAO {
	...
	public int endPage(int nowPage,int board_code) { // return:보여줄 마지막페이지 input:현재페이지
		final int BLOCK = 10; // 밑에 페이지 표기수
		int endPage = 0;
	...
	public int startPage(int nowPage) { // return:시작페이지 input:현재페이지 ;;;board_code
		final int BLOCK = 10; // 밑에 페이지 표기수
		int startPage = 0;
	...
	public int pageCount(int board_code) { //같은 board_code몇페이지 나올것인지 판단하여 반환
		int count = 0;
		int i=0;
		count = count(board_code);
		final int PAGE_NUM = 10; // 한페이지에 표시되는 글의 수
		if (count % PAGE_NUM != 0) {
			i++;
		}
		count /= PAGE_NUM;
	...
	public int count(int board_code) { // 반환값 :board_code 가 같은 글의 수
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		int count = 0;
